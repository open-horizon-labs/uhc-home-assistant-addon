...
This is the **Tier 1 (no ingress)** add-on — the UI opens in its own
browser tab at `http://<your-ha-host>:8088`, not embedded in the Home
Assistant dashboard. See [DOCS.md](DOCS.md) for the full install
...
add-on repository in **Settings → Add-ons → Add-on Store → ⋮ →
Repositories**, then install **Unified Hi-Fi Control** from the list.
